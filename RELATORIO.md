...
**Notação assintótica utilizada**

- **O** (Big-O): limite superior, usado para o pior caso.
- **Ω** (Big-Omega): limite inferior, usado para o melhor caso.
- **Θ** (Big-Theta): limite justo (superior e inferior ao mesmo tempo),
  usado para o caso médio. Quando O e Ω coincidem, o algoritmo é Θ.
...
|--------|-------------|-----------------|---------|
| Melhor | n(n-1)/2    | 0               | Ω(n²)   |
| Médio  | n(n-1)/2    | ≈ n(n-1)/4      | Θ(n²)   |
| Pior   | n(n-1)/2    | n(n-1)/2        | O(n²)   |
...
sempre percorrido por completo.
...
- Trocas: no máximo **1 troca por iteração externa**, logo no máximo (n-1)
  trocas em qualquer caso — O(n), independentemente da entrada.

**Complexidade de tempo**

| Caso   | Comparações | Trocas | Tempo   |
|--------|-------------|--------|---------|
| Melhor | n(n-1)/2    | ≤ n-1  | Ω(n²)   |
| Médio  | n(n-1)/2    | ≤ n-1  | Θ(n²)   |
| Pior   | n(n-1)/2    | ≤ n-1  | O(n²)   |
...
| Trocas (pior caso)             | n(n-1)/2 (O(n²))          | n-1 (O(n))      |
| Melhor caso (tempo)            | Ω(n²) (nesta versão)      | Ω(n²)           |
| Caso médio (tempo)             | Θ(n²)                     | Θ(n²)           |
| Pior caso (tempo)              | O(n²)                     | O(n²)           |

Ambos pertencem à mesma classe assintótica de tempo, Θ(n²). A diferença relevante entre eles está no
**número de escritas/trocas na memória**: o Bubble Sort pode realizar até
O(n²) trocas, enquanto o Selection Sort realiza no máximo O(n) trocas. Como
...
eles está no número de **trocas**: o Bubble Sort pode chegar a O(n²)
trocas no pior caso, enquanto o Selection Sort nunca ultrapassa O(n)
